...
const Footer = ({ arabicLanguage }) => {
  ...
  return (
    <footer className="py-10 relative z-10 mt-32 bg-[#0f101b]">
      <div className="flex flex-col gap-[30px] items-center justify-center mb-[60px] relative z-10">
        ...
        <ul className="flex flex-col items-center lg:flex-row gap-5">
          {links.map((link, i) => {
            return (
              <li key={i}>
                <a className="text-white text-xs" href={link.location}>
      ...
      <div className="footer-bottom container xl:max-w-[1328px] px-4 mx-auto border-t pt-[30px] pb-2 border-white border-opacity-30 relative z-10">
        <div className="flex flex-col items-center lg:flex-row gap-8 justify-between">
          {arabicLanguage ? (
            ...
            <p className={`text-[10px] lg:text-xs text-white text-center ${poppins.className}`}> © Copyright {new Date().getFullYear()}, All Rights Reserved by Logic Saint </p>
          )}
      ...
      <a className={`block ml-auto cursor-pointer fixed bottom-6 ${arabicLanguage ? 'left-6' : 'right-6'}`} href="https://api.whatsapp.com/send?phone=15551234567">
