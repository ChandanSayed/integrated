...
const Services = ({ arabicLanguage }) => {
  return (
    <div className="container xl:max-w-[1268px] px-4 mx-auto text-white">
      <div id="services" name="services">
        <h2 data-aos="fade-up" className="font-GilroyBlack font-black text-white text-center text-xl lg:text-5xl capitalize mb-2.5">
...
Understanding the risks your business might encounter is crucial. From global financial downturns to the surge in cyber threats, we detect risks early and take timely action. This is where our risk advisory specialists step in to provide unmatched expertise.`
          ...
          classes={'lg:flex-reverse lg:flex-row-reverse'}
          name="eight"
        />
...
`
        ...
        <TwoColumnSection arabicLanguage={arabicLanguage} lineOneColor="text-yellow" lineTwoColor="text-white" imgSrc={ten} lineOne={arabicLanguage ? 'تنفيذ ودعم وCDP والأعمال تطبيقات  ' : 'ERP, CRM, CDP '} lineTwo={arabicLanguage ? 'ERP وCRM ' : '& Business applications implementation & Support'} para={arabicLanguage ? `تعد المنصات الرقمية الأساسية بمثابة مفاتيح لرحلة التحول الرقمي الناجحة في أي مؤسسة. إن تحديد الاحتياجات ومقارنة الخيارات المتاحة في السوق وإدارة وتنفيذ التنفيذ الناجح ليست رحلة سهلة.` : `The foundational digital platforms are keys for a successful digital transformation journey in any organization. Identifying the needs, comparing the options in the market, managing and executing a successful implementation is not an easy journey.`} classes={'lg:flex-reverse lg:flex-row-reverse'} name="ten" />

        <TwoColumnSection arabicLanguage={arabicLanguage} lineOneColor="text-yellow" lineTwoColor="text-white" imgSrc={eleven} lineOne={arabicLanguage ? 'CIO كخدمة' : 'CIO as a '} lineTwo={arabicLanguage ? ' (vCIO)' : 'service (vCIO)'} para={arabicLanguage ? `هل لاحظت يومًا وجود انفصال بين متطلبات عملك وما تقدمه تكنولوجيا المعلومات فعليًا؟ حتى أدنى اختلال يمكن أن يؤثر بشكل كبير على قدرتك على خدمة العملاء والحفاظ على مشاركة موظفيك وفعاليتهم. سد هذه الفجوة مع مدير تكنولوجيا المعلومات كخدمة في شركة iT IT، والذي يجمع بين مزيج قوي من الفطنة التجارية والخبرة الفنية لأي مؤسسة بأي حجم.` : `Ever noticed a disconnect between your business requirements and what IT actually delivers? Even the slightest misalignment can significantly impact your ability to service customers and keep your staff engaged and effective. Bridge that gap with iiT IT’s CIO as a Service, which combines a potent mix of business acumen with technical expertise for any sized organization.          `} classes={'lg:flex-reverse lg:flex-row '} name="eleven" />

        <TwoColumnSection arabicLanguage={arabicLanguage} lineOneColor={arabicLanguage ? 'text-white' : 'text-yellow'} lineTwoColor={arabicLanguage ? 'text-yellow' : 'text-white'} imgSrc={twelve} lineOne={arabicLanguage ? ' CISO ' : 'CIO as a '} lineTwo={arabicLanguage ? 'كخدمة' : 'service (vCIO)'} para={arabicLanguage ? `مع تزايد كمية الهجمات السيبرانية وتعقيدها يوميًا، أصبح الأمن السيبراني أمرًا حيويًا بشكل متزايد. لا تستطيع معظم الشركات تحمل تكلفة فريق متخصص للأمن السيبراني، أو توفير التدريب، أو إدارة التقنيات اللازمة لتوفير الامتثال والحماية. كلما كانت التهديدات أكثر تعقيدًا، كلما كانت الاستجابة وخبرة المستجيبين أكثر تعقيدًا.` : `As the quantity and complexity of cyber-attacks grow daily, cybersecurity is becoming increasingly vital. Most businesses cannot afford a specialized Cybersecurity Team, provide training, or manage the necessary technologies to provide compliance and protection. The more sophisticated the threats are, the more complicated the response and expertise of the responders should be. `} classes={'lg:flex-reverse lg:flex-row-reverse'} name="twelve" />

        <TwoColumnSection arabicLanguage={arabicLanguage} lineOneColor={arabicLanguage ? 'text-white' : 'text-yellow'} lineTwoColor={arabicLanguage ? 'text-yellow' : 'text-white'} imgSrc={thirteen} lineOne={arabicLanguage ? 'السحابية' : 'Cloud '} lineTwo={arabicLanguage ? 'الاستشارات ' : 'Consulting'} para={arabicLanguage ? `تساعد الاستشارات السحابية الشركات على التعرف على فوائد الحوسبة السحابية وجني فوائدها وحل المشكلات باستخدام الحلول السحابية الحالية. يمكن لمستشاري Itransition السحابي مساعدتك في الانتقال إلى النظام الأساسي السحابي الذي تختاره بالإضافة إلى تطوير السحابة والتكامل وإعادة التصميم. تغطي خدماتنا مراجعة وإنشاء إستراتيجية السحابة، وتقييم البنية التحتية، والترحيل السحابي، وإعادة تصميم البنية، وتطوير السحابة أولاً، والتكامل السحابي، واختيار التكنولوجيا السحابية.` : `Cloud consulting helps companies recognize and reap the benefits of cloud computing and solve issues with their existing cloud solutions. Itransition’s cloud consultants can assist you with migration to the cloud platform of your choice as well as cloud development, integration, and redesign. Our services cover reviewing and creating the cloud strategy, infrastructure assessing, cloud migration, architecture redesign, cloud-first development, cloud integration, and cloud technology selection.           `} classes={'lg:flex-reverse lg:flex-row '} name="thirteen" />

        <TwoColumnSection arabicLanguage={arabicLanguage} lineOneColor={arabicLanguage ? 'text-white' : 'text-yellow'} lineTwoColor={arabicLanguage ? 'text-yellow' : 'text-white'} imgSrc={fourteen} lineOne={arabicLanguage ? 'النظام' : 'System Integration '} lineTwo={arabicLanguage ? 'تكامل  والتطوير المخصص' : '& Custom development'} para={arabicLanguage ? `في iT نقوم بتطوير منتجات برمجية متقدمة مصممة جيدًا، ومشفرة وفقًا لمعايير عالية، كما تم اختبارها جيدًا. نحن نضمن التواصل الذي لا تشوبه شائبة وتقديم تحديثات منتظمة. تغطي خدمات تطوير العملاء لدينا تقنيات مختلفة توفر الويب وتطبيقات الهاتف المحمول وبرامج المؤسسات وتطبيقات SaaS والواجهة الخلفية والواجهة الأمامية والتطورات عبر الأنظمة الأساسية.` : `At iiT we develop advanced software products that are well-designed, coded to high standards, and also well-tested. We ensure flawless communication & provide regular updates. Our custome development services cover different technologies that provide web, mobile apps, enterprise software, SaaS applications, backend & frontend, cross-platform developments.`} classes={'lg:flex-reverse lg:flex-row-reverse'} name="fourteen" />
